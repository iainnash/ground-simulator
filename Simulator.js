...
function set_pos(az, el) {
  ...
  status.active = true;
  console.log(
    "set_pos(" + az + ", " + el + "): target == " + JSON.stringify(target)
  );
}
...
function azDelta(targetAz) {
  // N.B. must be passed targetAz to allow setting anticlockwise before replacing target
  var result =
    status.stop == "north"
      ? deltaAtSpeed(status.az, targetAz, azSpeed)
      : deltaAtSpeed(
          (status.az + 180.0) % 360.0,
          (targetAz + 180.0) % 360.0,
          azSpeed
        ); // swap semi-circles for stop == south
  return result;
}
...
wss.on("connection", function connection(ws) {
  ...
  function statusUpdate() {
    if (status.active) {
      if (needToMove(status.az, target.az)) updateAz();
      else if (needToMove(status.el, target.el)) updateEl();
      else if (status.demonstrationMode) setRandomPos();
...
console.log("WS 5401 SS Running");
    ...
    socket.on("data", function(data) {
      ...
      if (dataString.localeCompare("p\n") == 0) {
        ...
        if (!status.active) {
          ...
          // report OK but take no action if already busy
          report(socket, RIG_OK);
        }
